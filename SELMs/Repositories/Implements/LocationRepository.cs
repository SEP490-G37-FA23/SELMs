...
namespace SELMs.Repositories.Implements
{
	public class LocationRepository : ILocationRepository
	{
		...
		public async Task RemoveRange(List<Location> locations)
		{
			db.Locations.RemoveRange(locations);
			await db.SaveChangesAsync();
		}

		public async Task<List<Location>> GetSublocationByParentId(int parentId)
		{
			return await db.Locations.Where(l => l.parent_location_id == parentId).ToListAsync();
		}
	}
}