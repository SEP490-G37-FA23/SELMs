using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;


namespace SELMs.Models.BusinessModel
{
    public class Argument
    {
        public int id { set; get; }
        public int group_id { set; get; }
        public string groupname { set; get; }
        public string tructhuoc { set; get; }
        public string color { set; get; }
        public string mac_address { set; get; }
        public int term { set; get; }
        public int thang { set; get; }
        public int nam { set; get; }
        public int sotrang { set; get; }
        public string tukhoa { set; get; }
        public string tukhoa3 { set; get; }
        public string tukhoa2 { set; get; }
        public string tukhoa1 { set; get; }
        public string tukhoa4 { set; get; }
        public string tukhoa5 { set; get; }
        public string tukhoa6 { set; get; }
        public string tukhoa7 { set; get; }
        public string tukhoa8 { set; get; }
        public string tukhoa9 { set; get; }
        public string tukhoa10 { set; get; }
        public string tukhoa11 { set; get; }
        public string tukhoa12 { set; get; }
        public string tukhoa13 { set; get; }
        public string sochungtu { set; get; }
        public string macongty { set; get; }
        public string ngayhen { set; get; }
        public string denngay { set; get; }
        public string tungay { set; get; }
        public string ngay { set; get; }
        public int start { set; get; }
        public int end { set; get; }
        public string mode { set; get; }
        public string calamviec { set; get; }
        public string date { set; get; }
        public string machinename { set; get; }
        public string product { set; get; }
        public int mcid { set; get; }
        public int MCID { set; get; }
        public int STID { set; get; }
        public int ng_qty { set; get; }
        public int error { set; get; }
        public string username { set; get; }
        public string makhachhang { set; get; }
        public bool isadmin { set; get; }
        public string password { set; get; }
        public string hovaten { get; set; }
        public string email { get; set; }
        public string sdt { get; set; }
        public string maphongban { get; set; }
        public string mahang { get; set; }
        public string exp { get; set; }
        public string value { get; set; }
        public string h { get; set; }
        public string hang { get; set; }
        public string machuan { get; set; }
        public string makho { get; set; }
        public string valueNhap { set; get; }
        public string expNhap { set; get; }
        public string valueXuat { set; get; }
        public string expXuat { set; get; }
        public string mapheduyet { set; get; }
        public string manhacungcap { set; get; }
        public string taikhoan { set; get; }
        public string dongiaban { set; get; }
        public int soluong { set; get; }
        public string tencongty { set; get; }
        public string mavanphong { set; get; }
        public string mancc { set; get; }
        public string tenhang { set; get; }
        public string manhomhang { set; get; }
        public string LoaiChungTu { set; get; }
        public string sotk { set; get; }
        public string dathanhtoan { set; get; }

        public string sotaikhoan { set; get; }
        public string loaitien { set; get; }

        public string congnoxau { set; get; }
        public string ngay1_7 { set; get; }
        public string ngay7_15 { set; get; }
        public string ngay15_30 { set; get; }
        public string ngay1_30 { set; get; }
        public string tren_30ngay { set; get; }
        public string tren_60ngay { set; get; }
        public string view { set; get; }
        public string ngay30_45 { set; get; }
        public string ngay45_60 { set; get; }
        public bool loc { set; get; }
        public string loai { set; get; }
        public string quy { set; get; }

    }
}