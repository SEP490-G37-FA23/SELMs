...
namespace SELMs.Api.Controllers
{
    ...
    public class ApiLocationController : ApiController
    {
        ...
        [HttpPost]
        [Route("locations/{locationId}/remove-member/{userCode}")]
        public async Task<IHttpActionResult> RemoveMember(int locationId, string userCode)
        {
            try
            {
                repository.RemoveLocationMember(locationId, userCode);
                return Ok();
            }
            catch (Exception ex)
            {
