...
app.controller('EquipmentAllocationReportCtrl', function ($scope, $http, $sce) {
    ...
    $scope.LoadEquipmentsReport = function () {
       
        $http.get(origin + '/api/v1/equipmentsAllocationReport').then(function (response) {
            $scope.ListEquips = response.data;
            $scope.sumEquips = response.data.length;
        });
    }

    $scope.LoadEquipmentsReport();
