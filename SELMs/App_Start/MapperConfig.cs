	...
	public static class MapperConfig
	{
		public static Mapper Initialize()
		{
			...
			var config = new MapperConfiguration(cfg =>
			{
				...
				cfg.CreateMap<Equipment_Handover_Form, EquipmentHandoverFormDTO2>()
				.ForMember(des => des.project_name, act => act.MapFrom(src => src.Project.project_name))
				.ForMember(des => des.handover, act => act.MapFrom(src => src.User.fullname))
				.ForMember(des => des.location_code, act => act.MapFrom(src => src.Location.location_code))
				.ForMember(des => des.create_date, act => act.MapFrom(src => src.create_date.ToString("dd/MM/yyyy")))
				.ForMember(des => des.handover_date, act => act.MapFrom(src => src.handover_date.Value.ToString("dd/MM/yyyy")))
				.ReverseMap();

				/*
				cfg.CreateMap<EquipmentHandoverFormDTO2, Equipment_Handover_Form>()
				.ForMember(des => des.create_date, act => act.MapFrom(src => DateTime.Parse(src.create_date)))
				.ForMember(des => des.handover_date, act => act.MapFrom(src => DateTime.Parse(src.handover_date)))
				.ForMember(des => des.receipt_date, act => act.MapFrom(src => DateTime.Parse(src.receipt_date)))
				.ReverseMap()
